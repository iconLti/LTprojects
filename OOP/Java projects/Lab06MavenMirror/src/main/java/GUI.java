...
public class GUI {
    ...
    private JMenuItem openItem, saveItem, genHTMLItem, genPDFItem;
    ...
    public void buildAndShowGUI() {
        ...
        genHTMLItem = new JMenuItem("Сгенерировать HTML отчет");
        genPDFItem = new JMenuItem("Сгенерировать PDF отчет");
        fileMenu.add(openItem);
        fileMenu.add(saveItem);
        fileMenu.add(genHTMLItem);
        fileMenu.add(genPDFItem);
        ...
        // default opened file //
        openedFile = new File("C:\\Users\\User\\Desktop\\LETI\\II\\OOP\\Java projects\\MainDocs\\AZ4o.xml");
        if (openedFile.exists()) {
            XMLfile.loadFromXML(tableModel, openedFile);
            JOptionPane.showMessageDialog(frame, "Данные успешно загружены из файла: " + openedFile.getName());
        } else {
            JOptionPane.showMessageDialog(frame, "Файл по умолчанию не найден. Пожалуйста, загрузите данные вручную.");
        }

        // generations pdf html

        genPDFItem.addActionListener(e -> {
            try {
                ReportGenerator.generatePDFReport();
                JOptionPane.showMessageDialog(null, "PDF отчет успешно создан!");
            } catch (JRException ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(null, "Ошибка при создании PDF отчета.");
            }
        });


        genHTMLItem.addActionListener(e -> {
            try {
                ReportGenerator.generateHTMLReport();
                JOptionPane.showMessageDialog(null, "HTML отчет успешно создан!");
            } catch (JRException ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(null, "Ошибка при создании HTML отчета.");
            }
        });
